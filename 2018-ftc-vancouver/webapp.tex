...
The archive is freely accessible under the CC-BY license.
...
Our project is not the first involving community-driven corpus annotation. We
...
bring different challenges and dictate different approaches. Marge
(2009)\cite{5494979} investigates using The Mechanical Turk to obtain audio
transcriptions. Mihalcea (2004)\cite{mihalcea2004building} offers a web
interface for word-sense disambiguation but focuses mostly on annotator conflict
resolution.
...
that is to be transcribed, should be should be transcribed with respect to
...
untranscribed, including noise or speech by other persons\footnote{We may later
add a speaker annotation}. Incomprehensible words are to be left uncorrected
...
A submitted transcribed segment always overwrites the present version but we
keep all the submissions in a database, so undo operations, clustering
submissions by their author etc. are possible but we had little need for this so
far.
